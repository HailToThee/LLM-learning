...
To transformers:
x = transformer(x)
x = [e1, e2, e3, e4] -> [t1, t2, t3, t4]

router H:
posibility = router(x) (l*num_experts)

raw_gates = posibility.softmax(dim=-1) #l*num_experts
...
desity_1 = mask_1.mean(dim=-2)  #equal to divide by batch_size
!--important
auxliary loss
density_1_proxy = raw_gates

auxilairy_loss = (density_1_proxy * density_1_proxy).mean()* float(num_gates ** 2)
```